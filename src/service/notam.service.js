...
let NotamsService = class NotamsService {
    ...
    targetFirs = ['SBCW', 'SBBS', 'SBRE', 'SBAZ', 'SBAO'];
    ignoredQCodes = new Set(['QAFTT']);
    restrictedSubjects = new Set([
        'RA',
        'RD',
        'RM',
        'RO',
    ...
    isNotamWithinCurrentWindow(item, now = new Date()) {
    ...
    buildAiswebUrl(icaocode, minutes) {
    ...
    textUsesBorderClosure(text) {
        const value = String(text ?? '').toUpperCase();
        if (!value)
            return false;
        return (value.includes('SEGUINDO TODA FAIXA DE FRONTEIRA TERRESTRE') ||
    ...
    inferAreaKindFromQCode(qcode, textE) {
        const subject = this.getQCodeSubject(qcode);
        switch (subject) {
            case 'RA':
    ...
    async fetchRemoteNotamsXml(icaocode, minutes, maxAttempts = 4, timeoutMs = 20000) {
    ...
    async fetchRemoteNotams(icaocode, minutes) {
    ...
    async fetchRemoteNotamsFromAllTargetFirs(minutes) {
        const allItems = [];
    ...
    bearingFromCenter(center, point) {
    ...
    destinationPoint(center, bearingDeg, distanceMeters) {
        ...
        const lon2 = lon1 +
            Math.atan2(Math.sin(bearing) * sinAd * cosLat1, cosAd - sinLat1 * Math.sin(lat2));
        return [this.toDegrees(lat2), this.toDegrees(lon2)];
    }
    interpolateArcPoints(center, radiusMeters, startPoint, endPoint, clockwise, steps = 48) {
        ...
        for (let i = 1; i < steps; i++) {
            ...
            const bearing = clockwise
                ...
                : startBearing - sweep * fraction;
            result.push(this.destinationPoint(center, this.normalizeBearing(bearing), radiusMeters));
        }
        return result;
    }
    parseTextEWithArcs(raw) {
        ...
        if (!text)
            return [];
        const allCoordTokens = text.match(/\d{6}(?:\.\d+)?[NS]\/?\d{7}(?:\.\d+)?[EW]/gi) ?? [];
        ...
        const arcRegex = /AO LONGO DE UM ARCO NO SENTIDO (HORARIO|ANTI-HORARIO) DE ([\d.,]+)\s*NM DE RAIO COM CENTRO EM (\d{6}(?:\.\d+)?[NS]\/?\d{7}(?:\.\d+)?[EW])/gi;
        ...
        for (const arcMatch of arcMatches) {
            ...
            if (coordIndex >= parsedCoords.length)
                break;
            ...
            const arcPoints = this.interpolateArcPoints(center, radiusNm * this.NM_TO_M, arcStart, arcEnd, direction === 'HORARIO', 48);
    ...
    toLatLonPair(coordinate) {
        return [Number(coordinate[1]), Number(coordinate[0])];
    }
    extractAeroviaName(properties) {
        const candidates = [
            properties?.nome,
            properties?.name,
            properties?.ident,
    ...
    geometryToLines(geometry) {
        ...
        if (geometry.type === 'MultiLineString') {
            return geometry.coordinates.map((line) => line.map((point) => this.toLatLonPair(point)));
        }
        return [];
    }
    async loadAerovias(url) {
    ...
    parseRplRecords(text) {
        ...
        for (const rawLine of lines) {
            ...
            if (!trimmed)
                continue;
            if (this.shouldSkipRplLine(trimmed))
                continue;
            const isLinhaPrincipal = /^\d{6}\s+\d{6}\s+\d{7}\s+[A-Z]{2,3}\d{1,4}[A-Z]?\s+/i.test(trimmed);
            if (isLinhaPrincipal) {
                if (atual) {
    ...
    shouldSkipRplLine(line) {
        const upper = line.toUpperCase();
        if (!upper)
            return true;
        if (upper.includes('PLANOS DE VOO REPETITIVOS'))
            return true;
        if (upper.includes('CLASSIFICA'))
            return true;
        ...
        if (upper.includes('PAG.:'))
            return true;
        if (upper.includes('VALIDO VALIDO DIAS OP'))
            return true;
        if (upper.includes('DEST') && upper.includes('OBSERVACOES'))
            return true;
    ...
    parseRplRecord(registro, airportMap, waypointMap) {
        const partes = registro.split(/\s+/).filter(Boolean);
        if (partes.length < 8)
            return null;
        const identInfo = this.extractFlightNumber(partes);
        if (!identInfo.ident || identInfo.index < 0)
            return null;
        const tipoAnv = identInfo.index + 1 < partes.length
            ? this.cleanAircraftType(partes[identInfo.index + 1])
            : '';
        const adepInfo = this.findFirstAdep(partes, identInfo.index + 2);
        if (!adepInfo)
            return null;
        const idxVel = partes.findIndex((token, idx) => idx > adepInfo.index && /^N\d{4}$/i.test(token));
        if (idxVel < 0)
            return null;
        const velocidade = partes[idxVel].toUpperCase();
        const nivelVoo = idxVel + 1 < partes.length ? this.extractFlightLevel(partes[idxVel + 1]) : '';
        ...
        const coords = this.resolveRouteCoordinates(adepInfo.icao, rotaTokens, destInfo.icao, airportMap, waypointMap);
    ...
    resolveRouteCoordinates(origem, rotaTokens, destino, airportMap, waypointMap) {
        const points = this.buildRoutePointSequence(origem, rotaTokens, destino, airportMap, waypointMap);
        const coords = [];
    ...
    buildRoutePointSequence(origem, rotaTokens, destino, airportMap, waypointMap) {
        const points = [];
    ...
    pushUniquePoint(points, value) {
        const normalized = String(value ?? '').trim().toUpperCase();
        if (!normalized)
            return;
        if (points[points.length - 1] === normalized)
            return;
        points.push(normalized);
    }
    findFirstAdep(partes, startIndex) {
    ...
    findDestinationFromTail(partes, routeStartIndex) {
        for (let i = partes.length - 1; i >= routeStartIndex; i--) {
            ...
            if (found) {
                ...
                if (prev.startsWith('EET/'))
                    continue;
                if (next.startsWith('EET/'))
                    continue;
                return {
                    ...found,
                    index: i,
    ...
    isValidRouteToken(token) {
        if (!token)
            return false;
        if (token === 'DCT' || token === 'IFR' || token === 'VFR' || token === 'C' || token === 'NIL')
            return false;
        if (/^N\d{4}$/i.test(token))
            return false;
        if (/^M\d{3}$/i.test(token))
            return false;
        if (/^F\d{3,4}$/i.test(token))
            return false;
        if (/^\d{2,4}$/.test(token))
            return false;
        if (/^[A-Z]{4}\d{4}$/.test(token))
            return false;
        if (this.isAirwayToken(token))
            return false;
        if (/^EQPT$/i.test(token))
            return false;
        if (/^PBN$/i.test(token))
            return false;
        if (/^EET$/i.test(token))
...
exports.NotamsService = NotamsService;
exports.NotamsService = NotamsService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [env_service_1.EnvService])
], NotamsService);
//# sourceMappingURL=notam.service.js.map